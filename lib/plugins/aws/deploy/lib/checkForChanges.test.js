...
describe('checkForChanges', () => {
  ...
  describe('#checkIfDeploymentIsNecessary()', () => {
    ...
    it('should not set set a flag if the remote and local files are different', () => {
      ...
      const input = [
        { Metadata: { filesha256: 'some-different-remote-hash' } },
        { Metadata: { filesha256: 'some-different-remote-hash' } },
        { Metadata: { /* no filesha256 available */ } },
      ];
      normalizeCloudFormationTemplateStub.returns({ foo: 'bar' });
    ...
    it('should set set a flag to not re-deploy if the remote and local files are different', () => {
      // eji/gfOD9pQzrW6QDTWz4jhVk/dqe3q11DVbi6Qe4ks= <-- hash of the local CloudFormation file
      // awnY7Oi280gp5kTCloXzsqJCO4J766x6hATWqQsN/uM= <-- hash of the local zip file
      const input = [
        { Metadata: { filesha256: 'eji/gfOD9pQzrW6QDTWz4jhVk/dqe3q11DVbi6Qe4ks=' } },
        { Metadata: { filesha256: 'awnY7Oi280gp5kTCloXzsqJCO4J766x6hATWqQsN/uM=' } },
      ];
      normalizeCloudFormationTemplateStub.returns({ foo: 'bar' });
      globbySyncStub.returns(['my-service.zip']);
      readFileSyncStub.returns(new Buffer('my-service.zip content'));
      ...
      return expect(awsDeploy.checkIfDeploymentIsNecessary(input))
        .to.be.fulfilled.then(() => {
          ...
          expect(globbySyncStub).to.have.been.calledOnce;
          expect(readFileSyncStub).to.have.been.calledOnce;
          expect(awsDeploy.serverless.cli.log).to.have.been.calledWithExactly(
